...
test("buildDomainReportRows distinguishes explicit not-applicable answers from hidden follow-up scales", () => {
	const instrument = buildInstrument();
	const auditSession = buildAuditSession();

	const movementDomain = buildDomainReportRows(auditSession, instrument).find(row => row.domainKey === "movement");
